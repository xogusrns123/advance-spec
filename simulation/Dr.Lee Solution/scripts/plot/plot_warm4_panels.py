#!/usr/bin/env python3
"""WARM4_vs_gain split into 3 independent images (workload / subtask / segment),
no annotations/comment text — just points, axes, rho in title. Same data
(gapmetric_candidates.json), warm4 vs compose-over-hybrid gain.
"""
...
def base(ax):
    ...
    ax.grid(alpha=0.2)
    ax.set_xlabel("warm4  =  suffix-copyable share  P(s ≥ 4)   (%)", fontsize=10.5)
...
for r in rows:
    ax.scatter(100 * r[M], r["gain"], s=120, c=WLC[r["wl"]], edgecolor="k",
               linewidth=0.6, zorder=3)
rho = spearman([r[M] for r in rows], [r["gain"] for r in rows])
ax.set_title("per workload", fontsize=12, fontweight="bold")
base(ax); wl_legend(ax, {r["wl"] for r in rows})
fig.tight_layout(); fig.savefig(OUT / "WARM4_vs_gain_workload.png", dpi=150, bbox_inches="tight")
plt.close(fig)
...
rows = D["subtask"]
fig, ax = plt.subplots(figsize=(7.0, 5.4))
xs = [100 * r[M] for r in rows]; ys = [r["gain"] for r in rows]
for r in rows:
    ax.scatter(100 * r[M], r["gain"], s=55, c=WLC[r["wl"]], edgecolor="k",
...
base(ax); wl_legend(ax, {r["wl"] for r in rows}, extra=[fitline])
fig.tight_layout(); fig.savefig(OUT / "WARM4_vs_gain_category.png", dpi=150, bbox_inches="tight")
plt.close(fig)
...
fig, ax = plt.subplots(figsize=(7.0, 5.4))
for r in rows:
    ax.scatter(100 * r[M], r["gain"], s=90, c=WLC[r["wl"]], marker=SEG_MARK[r["seg"]],
               edgecolor="k", linewidth=0.5, zorder=3)
rho = spearman([r[M] for r in rows], [r["gain"] for r in rows])
segh = [plt.Line2D([0], [0], marker=m, ls="", mfc="#999", mec="k", ms=9, label=sg)
        for sg, m in SEG_MARK.items()]
wl_legend(ax, {r["wl"] for r in rows}, extra=segh, loc="lower right")
ax.set_title("per segment", fontsize=12, fontweight="bold")
base(ax)
fig.tight_layout(); fig.savefig(OUT / "WARM4_vs_gain_segment.png", dpi=150, bbox_inches="tight")
plt.close(fig)
print("saved 3 panels -> WARM4_vs_gain_{workload,subtask,segment}.png")
