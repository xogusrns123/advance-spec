...
- `full` (6-tool): 일반 LLM (Qwen3-8B/14B/32B, Llama3-8B 등) 에 사용. `run_pipeline.sh` 의 Stage 1 default.
...
서버 startup 시점에 `SGLANG_ORACLE_VANILLA=1` 이 빠져 있으면, agent 들은 에러 없이 종료하지만 모든 step 에 `spec_decode` 가 결락된다. 이것이 Stage 1 의 가장 흔한 silent failure 이다; 손으로 다시 돌릴 때 `run_parallel_stage1.sh:49` 가 env 에 들어 있는지 반드시 재확인하라.
