...
1. Pipeline shell이 `SGLANG_ORACLE_VANILLA=1` (그리고 필요시 `SGLANG_ORACLE_REPLAY=…`, `SGLANG_LATENCY_ONLY=1` 등) 을 export.
   - 예: `simulation/scripts/run_parallel_stage1.sh:49`, `_stage1_round_robin.sh:67`.
2. Pipeline이 `python3 -m simulation.oracle.install_hook -- <sglang args>` 를 실행.
...
| 변수 | 설정자 | 사용자 | 의미 / 부작용 |
|------|--------|--------|---------------|
| `SGLANG_ORACLE_VANILLA` | Pipeline shell (e.g. `simulation/scripts/run_parallel_stage1.sh:49`, `_stage1_round_robin.sh:67`) | `install_hook.install_oracle_patch`, 워커 `__init__` (Tier 1로 박힌 코드), `oracle_patch.is_oracle_enabled` | `=1` 일 때만 oracle hook 이 실제로 install. 미설정/`=0` 이면 SGLang 정상 동작. |
| `SGLANG_ORACLE_REPLAY` | Pipeline (Stage 3c 등) | `oracle_patch.py:39` 에서 path 로 load → `TrajectoryState` 활성 | path 가 truthy 면 매 step `req.output_ids[-1]` 을 trajectory 토큰으로 강제. Stage 1 에서는 `unset` 처리 (`run_parallel_stage1.sh:52`). |
| `SGLANG_ORACLE_LOG` | `simulation/scripts/_run_topk_sweep_*.sh` 등 | `oracle_patch.py:36` | per-step JSONL 경로 (default `/tmp/sglang_oracle_vanilla.jsonl`). 동시 서버 인스턴스를 띄울 때 충돌 방지용. |
...
| `SGLANG_ALLOW_OVERWRITE_LONGER_CONTEXT_LEN` | `docker-compose.yml:11`, `Dockerfile:63`, `simulation/scripts/bench_eagle3_configs.py:313` | SGLang `srt/configs/model_config.py:380` | EAGLE3 draft model 의 max_position_embeddings 가 target context_length 보다 짧을 때 overwrite 허용 (없으면 ValueError). README:864 에 "EAGLE3 context_length 불일치" 트러블슈팅 등록. |
| `TORCHINDUCTOR_COMPILE_THREADS` | `install_hook.py:25-26`, `_stage1_round_robin.sh:69`, `hybrid_spec_decoding/sglang_integration/suffix_worker.py:23-24` | torch.compile 내부 | torch import 전에 `=1` 을 강제해 inductor compile worker fork 폭증을 방지. Pipeline에서 fork bomb watchdog 과 더불어 자식 프로세스 수를 안정화. |
